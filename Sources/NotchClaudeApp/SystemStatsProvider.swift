...
final class SystemStatsProvider: ObservableObject {
    ...
    private func refreshNetworkText() {
        ...
        uploadText = formatBytesPerSecond(Double(sample.sentBytes - previousSample.sentBytes) / interval)
        downloadText = formatBytesPerSecond(Double(sample.receivedBytes - previousSample.receivedBytes) / interval)
        previousNetworkSample = sample
    }
